...
public class TCPMonitor extends JFrame {
     ...
     * @param embedded
     */
    public TCPMonitor(int listenPort, String targetHost, int targetPort, boolean embedded) {
        super(getMessage("httptracer00","TCP Monitor"));
     ...
     * @param targetPort
     */
    public TCPMonitor(int listenPort, String targetHost, int targetPort) {
    ...
    public static void main(String[] args) {
        try {
            ...
            if (args.length == 3) {
                ...
                new TCPMonitor(p1, args[1], p2);
            } else if (args.length == 1) {
                int p1 = Integer.parseInt(args[0]);
                new TCPMonitor(p1, null, 0);
            } else if (args.length != 0) {
                System.err.println(
                        getMessage("usage00", "Usage:")
                        + " TCPMon [listenPort targetHost targetPort]\n");
            } else {
                new TCPMonitor(0, null, 0);
            }
        } catch (Throwable exp) {
            exp.printStackTrace();
        }
