...
class Listener extends JPanel {
   ...
   public class Columns{
      ...
      public static final int ELLAPSED_TIME=2;
      public static final int BYTES_PER_SECOND=3;
    ...
    public Listener(JTabbedPane _notebook, String name, int listenPort,
        ...
        notebook = _notebook;
        if (name == null) {
            name = TCPMonitor.getMessage("port01", "Port") + " " + listenPort;
        }
        ...
        final String start = TCPMonitor.getMessage("start00", "Start");
        top.add(stopButton = new JButton(start));
        top.add(Box.createRigidArea(new Dimension(5, 0)));
        top.add(new JLabel("  "
                + TCPMonitor.getMessage("listenPort01", "Listen Port:")
                + " ", SwingConstants.RIGHT));
        top.add(portField = new JTextField("" + listenPort, 4));
        top.add(new JLabel("  " + TCPMonitor.getMessage("host00", "Host:"),
                SwingConstants.RIGHT));
        top.add(hostField = new JTextField(host, 30));
        top.add(new JLabel("  " + TCPMonitor.getMessage("port02", "Port:") + " ",
                SwingConstants.RIGHT));
        top.add(tPortField = new JTextField("" + targetPort, 4));
        top.add(Box.createRigidArea(new Dimension(5, 0)));
        top.add(isProxyBox = new JCheckBox(TCPMonitor.getMessage("proxy00", "Proxy")));
        ...
        stopButton.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent event) {
                if (TCPMonitor.getMessage("stop00",
        ...
        tableModel = new DefaultTableModel(new String[]{
           "State","Time","Elapsed Time",
           "Bytes/Second", "Originating Host", "Bytes Received"}, 0);
        tableModel.addRow(new Object[]{"---",
                                       TCPMonitor.getMessage("mostRecent00",
                                               "Most Recent"),
                                       "---", "---", "---", "---"});
        ...
        TableColumn col;
        col = connectionTable.getColumnModel().getColumn(Columns.STATE);
        col.setMaxWidth(col.getPreferredWidth() / 2);
        col = connectionTable.getColumnModel().getColumn(Columns.HOST);
        ...
        final String removeSelected = TCPMonitor.getMessage("removeSelected00",
                "Remove Selected");
        buttons.add(removeButton = new JButton(removeSelected));
        buttons.add(Box.createRigidArea(new Dimension(5, 0)));
        final String removeAll = TCPMonitor.getMessage("removeAll00", "Remove All");
        ...
        leftPanel.add(new JLabel("  "
                + TCPMonitor.getMessage("request01", "Request")));
        leftPanel.add(new JLabel(" "
                + TCPMonitor.getMessage("wait01",
                        "Waiting for connection")));
        rightPanel = new JPanel();
        rightPanel.setLayout(new BoxLayout(rightPanel, BoxLayout.Y_AXIS));
        rightPanel.add(new JLabel("  "
                + TCPMonitor.getMessage("response00", "Response")));
        rightPanel.add(new JLabel(""));
        ...
        bottomButtons.add(
                xmlFormatBox =
                new JCheckBox(TCPMonitor.getMessage("xmlFormat00", "XML Format")));
        xmlFormatBox.setEnabled(false);
        bottomButtons.add(Box.createRigidArea(new Dimension(5, 0)));
        final String save = TCPMonitor.getMessage("save00", "Save");
        bottomButtons.add(saveButton = new JButton(save));
        bottomButtons.add(Box.createRigidArea(new Dimension(5, 0)));
        final String resend = TCPMonitor.getMessage("resend00", "Resend");
        bottomButtons.add(resendButton = new JButton(resend));
        resendButton.setEnabled(false);
        bottomButtons.add(Box.createRigidArea(new Dimension(5, 0)));
        final String switchStr = TCPMonitor.getMessage("switch00", "Switch Layout");
        bottomButtons.add(switchButton = new JButton(switchStr));
        bottomButtons.add(Box.createHorizontalGlue());
        final String close = TCPMonitor.getMessage("close00", "Close");
    ...
    public void start() {
        ...
        notebook.setTitleAt(i, TCPMonitor.getMessage("port01", "Port") + " " + port);
        int tmp = Integer.parseInt(tPortField.getText());
        tPortField.setText("" + tmp);
        sw = new SocketWaiter(this, port);
        stopButton.setText(TCPMonitor.getMessage("stop00", "Stop"));
        portField.setEditable(false);
        hostField.setEditable(false);
        tPortField.setEditable(false);
    ...
    public void stop() {
        try {
            ...
            sw.halt();
            stopButton.setText(TCPMonitor.getMessage("start00", "Start"));
            ...
            try {
                ...
                for (Iterator i = connections.iterator(); i.hasNext();
                    ...
                    if (lsm.isSelectedIndex(n + 1)
                        ...
                        out.write("\n==============\n".getBytes());
                        out.write(((TCPMonitor.getMessage("listenPort01",
                                "Listen Port:")
                                + " " + rc + "\n")).getBytes());
                        out.write((TCPMonitor.getMessage("targetHost01",
                                "Target Host:")
                                + " " + hostField.getText()
                                + "\n").getBytes());
                        rc = Integer.parseInt(tPortField.getText());
                        out.write(((TCPMonitor.getMessage("targetPort01",
                                "Target Port:")
                                + " " + rc + "\n")).getBytes());
                        out.write((("==== "
                                + TCPMonitor.getMessage("request01", "Request")
