...
class Connection extends Thread {
    ...
    public void run() {
        try {
            ...
            String dateformat = TCPMonitor.getMessage("dateformat00", "yyyy-MM-dd HH:mm:ss");
            DateFormat df = new SimpleDateFormat(dateformat);
            connectionTime = new Date();
            int count = listener.connections.size();
            listener.tableModel.insertRow(count + 1,
                    new Object[]{
                        TCPMonitor.getMessage("active00","Active"),
                        df.format(connectionTime),
                        "",
                        "",
                        fromHost,
                        ""});
...
//
//            		if (rr2 != null) {
//            			listener.tableModel.setValueAt(rr2.getElapsed(), 1 + index, TCPMonitor.ELAPSED_COLUMN);
//            		}
//
...
//                                TCPMonitor.getMessage("resp00", "Resp"), 1 + index,
//                                TCPMonitor.STATE_COLUMN);
//                    }
//                    rr1 = null;
//                }
//
...
//                    if ((index >= 0) && (rr1 != null)) {
//                        listener.tableModel.setValueAt(
//                                TCPMonitor.getMessage("req00", "Req"), 1 + index,
//                                TCPMonitor.STATE_COLUMN);
//                    }
//                    rr2 = null;
//                }
...
//            }
            ...
            if (index >= 0) {
                listener.tableModel.setValueAt(
                        TCPMonitor.getMessage("done00", "Done"),
                        1 + index, Listener.Columns.STATE);
            }
            updateView(index);
        } catch (Exception e) {
            ...
            if (index >= 0) {
                listener.tableModel.setValueAt(
                        TCPMonitor.getMessage("error00", "Error"), 1 + index,
                        Listener.Columns.STATE);
            }
   ...
   protected void updateView(int index) {
      inputText.setText(inputBuffer.toString());
      int bytesReceived = inputBuffer.length();
      listener.tableModel.setValueAt(Integer.toString(inputBuffer.length()), index + 1, Listener.Columns.DATA_LENGTH);
      Date now = new Date();
      long diff = now.getTime() - connectionTime.getTime();
      double bps = (double) bytesReceived / ((double) diff / 1000.0);
      listener.tableModel.setValueAt(Double.toString(bps), index + 1, Listener.Columns.BYTES_PER_SECOND);
      listener.tableModel.setValueAt(Long.toString(diff), index + 1, Listener.Columns.ELLAPSED_TIME);
   }
